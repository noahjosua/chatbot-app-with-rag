...
import constants
import constants_rephrase_on_chat_history
...
def chat_flow(chat_model, retriever):
    ...
    if user_prompt := st.chat_input(constants.CHAT_INPUT):
        ...
        with ((st.spinner(constants.SPINNER_LABEL))):
            ...
            contextualize_system_prompt_template = PromptTemplate.from_template(constants_rephrase_on_chat_history.CONTEXTUALIZED_SYSTEM_PROMPT)
            contextualize_chain = LLMChain(llm=chat_model, prompt=contextualize_system_prompt_template, callbacks=None,
                                           verbose=True)
            rephrased_user_prompt = contextualize_chain.invoke({'chat_history': chat_history, 'user_prompt': user_prompt})['text']
